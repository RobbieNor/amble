...
class LectureNoteCreatorWindow(Window):
    ...
    def on_createnote_clicked(self,widget):
        ...
        print app_loc
        application = imp.load_source('applications', app_loc )
        ...
        #NOTE: if a new application is added later, we shouldnt have to account for pdf generation if not supported
        #..., the ui should force the checkbox to be unticked.
        if options['save_pdf_button'] == True:
            genpdf_line = application.createPDFLine(path)
            subprocess.call(genpdf_line, shell=True)
        if options['gen_html_button'] == True:  
            #Generate HTML
            x = 1 #
        to_be_backed_up = []
        if options['bkp_pdf_button'] == True:
            #to_be_backed_up.append('.pdf')
            x = 1 #
        if options['bkp_xoj_button'] == True:
            to_be_backed_up.append(application.getExtension())
        if options['bkp_html_button'] == True:
            x = 1 #            
            #to_be_backedup.append(.html)
        backup_file = None
        if options['compress_button'] == True:
            bkp_com = self.__compressiondir__ +"/" + str(default_values['bkpcom_chooser']) + ".py"
            compressor = imp.load_source('compressions' , bkp_com)
            backup_file = compressor.compress(to_be_backed_up, location, title) 
        else:
            os.makedirs(location + "/" + title)
            for extension in to_be_backed_up:
                shutil.copy(location + "/"+title + extension,location + "/"+title + "/" + title +  extension)
            backup_file = location + "/" + title 
        if options['bkploc_same_button'] == True:
            try:
            ...
            #REMEMBER: WHEN bkploc_same is activated, deactive the bkp_chooser ui
            #TODO: make the date folder the compressed one?
            bkploc = defaults['bkploc_chooser']
        ...
        if options['html_index_button'] == True:
            x = 1 #
            #Regenerate index              
        ...
        #Do this last
        if options['save_xoj_button'] == False:
            os.remove(path)
        if options['save_pdf_button'] == False:
